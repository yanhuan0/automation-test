...
public class CompareUtil extends BaseWebDriverTest {

	private static StringReader filterNonAnsiFile(FileReader fileReader) throws IOException { // added by xbai on
																								// 08/27/2010
		int c;
	...
	/**
	 * Compare text-format file with its base file. A diff file will be generated if
	...
	public void compareTextFiles(String fileName) throws Exception, IOException {
		...
		resultFilePath = getResultDir() + "/" + fileName;
	...
	public void compareTextFiles(String baseFilePath, String resultFilePath) throws Exception, IOException {
		...
		diffFilePath = getBaseDir() + "/" + diffName + ".diff";
		diffFile = new File(diffFilePath);

		if (!baseFile.exists()) {
			logError("compareTextFile() - Can't find base file: " + baseFilePath);
			return;
		}

		if (!resultFile.exists()) {
			logError("compareTextFile() - Can't find result file: " + resultFilePath);
			return;
		}
		...
		// Read file
		BufferedReader baseBuffReader = new BufferedReader(filterNonAnsiFile(new FileReader(baseFile))); // updated by
																											// xbai on
																											// 08/27/2010
		BufferedReader resultBuffReader = new BufferedReader(filterNonAnsiFile(new FileReader(resultFile))); // updated
																												// by
																												// xbai
																												// on
																												// 08/27/2010
			...
			if (baseLine.compareTo(resultLine) != 0) {
				if (!diffFile.exists()) {
					...
					writer.write(
							"==============================================================================================================="
									+ LINE_SEPARATOR);
					result = "The result file " + resultFilePath + " and base file " + baseFilePath
							+ " are different. The diff is generated: " + diffFilePath + LINE_SEPARATOR;
				}
		...
		if (hasDifference) {
			...
			logInfo("The result file " + resultFilePath + " and base file " + baseFilePath + " are identical");
	...
	public boolean comparePDFFiles(String pdfFile1, String pdfFile2, double threshold) throws Exception {
		// m_logger.logMethod(this.getClass().getName(), "comparePDFFiles", new
		// String[]{pdfFile1, pdfFile2});
		...
		if (!baseFile.exists()) {
			logError("comparePDFFiles() - Can't find base file: " + file1);
			return false;
		}
		if (!resultFile.exists()) {
			logError("comparePDFFiles() - Can't find result file: " + file2);
			return false;
		}
		...
		try {
			...
			if (!doc1.equalsIgnoreCase(doc2)) {
				...
				logInfo("Result text:");
				logInfo(doc2);
				logError("Text comparison: FALSE");
			} else
				logInfo("Text are indentical");
			...
			// System.out.println("Page Count Comparision : "+ (pageImages1.length ==
			// pageImages2.length) );
			if (pageImages1.length == pageImages2.length)
				logInfo("Page count are same");
			else {
				logInfo("Base page count:" + pageImages1.length);
				logInfo("Result page count:" + pageImages2.length);
				logError("Page count are not same");
			}
			...
			for (int i = 0; i < pageImages1.length; i++) {
				if (!checkIdentical(pageImages1[i], pageImages2[i], resultImageFileName, threshold)) {
					...
					pdDoc1.close();
					pdDoc2.close();
					logError("Page images are not identical");
					return false;
				}
			}
			// System.out.println("Page Image Comparision : true");
			logInfo("Page images are identical");
			...
			logError("Exception in compare PDF files. " + "Exception: " + e.getMessage());
			return false;
		} finally {
			pdDoc1.close();
			pdDoc2.close();
	...
	/**
	 * Compare two Briefing book in PDF files with system time. For example,
	 * charttest.pdf
	 * 
	...
	public boolean comparePDFFilesWithTime(String pdfFile) throws Exception {
		...
		String file1 = getBaseDir() + "/" + pdfFile;
		String file2 = getBaseDir() + "/" + pdfFile;
	 ...
	 * @param pdfFile2 Result PDF file full path
	 */
	public boolean comparePDFFilesWithTime(String pdfFile1, String pdfFile2) throws Exception {
		// m_logger.logMethod(this.getClass().getName(), "comparePDFFiles",
		// new String[] { pdfFile1, pdfFile2 });
		...
		try {
			...
			while (doc1.indexOf("Time run:") > 0) {
				if (doc1.indexOf(" PM") > 0) {
					tempDoc1 = doc1.substring(doc1.indexOf("Time run:"), doc1.indexOf(" PM")) + " PM";
				} else if (doc1.indexOf(" AM") > 0) {
					tempDoc1 = doc1.substring(doc1.indexOf("Time run:"), doc1.indexOf(" AM")) + " AM";
				}
			...
			while (doc2.indexOf("Time run:") > 0) {
				if (doc2.indexOf(" PM") > 0) {
					tempDoc2 = doc2.substring(doc2.indexOf("Time run:"), doc2.indexOf(" PM")) + " PM";
				} else if (doc2.indexOf(" AM") > 0) {
					tempDoc2 = doc2.substring(doc2.indexOf("Time run:"), doc2.indexOf(" AM")) + " AM";
				}
	...
	public boolean comparePDFFiles(String pdfFile, double threshold) throws Exception {
		...
		String file1 = getBaseDir() + "/" + pdfFile;
		String file2 = getResultDir() + "/" + pdfFile;

		return comparePDFFiles(file1, file2, threshold);
	}

	/**
	 * Compare two Briefing book in PDF files. For Briefing book, the first page
	 * will be ignore since it has time stemp
	 *
	 * @param pdfFile1 Base PDF file full path
	 * @param pdfFile2 Result PDF file full path
	 * 
	...
	public boolean compareBriefingBookInPDFFiles(String pdfFile1, String pdfFile2) throws Exception {
		// m_logger.logMethod(this.getClass().getName(),
		// "compareBriefingBookInPDFFiles", new String[]{pdfFile1, pdfFile2});
		...
		if (!baseFile.exists()) {
			logError("compareBriefingBookPDFFiles() - Can't find base file: " + file1);
			return false;
		}
		if (!resultFile.exists()) {
			logError("compareBriefingBookPDFFiles() - Can't find result file: " + file2);
			return false;
		}
		...
		try {
			...
			logError("Exception in compareBriefingBookPDFFiles. " + "Exception: " + e.getMessage());
			return false;
		} finally {
			parser1 = null;
			parser2 = null;
	...
	/**
	 * Compare a page from result PDF file with its base file. For Briefing book,
	 * the first page will be ignore since it has time stemp
	 *
	...
	/**
	 * Compare a page from direct download PDF file with its base file. For Briefing
	 * book, the first page will be ignore since it has time stemp
	 *
	 * @param pdfFile PDF file name. For example, charttest.pdf
	 * 
	 */
	public boolean compareDirectDownBriefingBookInPDFFiles(String filePath) throws Exception {
		if (System.getProperty("biee.result.mode").equalsIgnoreCase("false"))
			return false;
		String fileName;
		...
		String file1 = getBaseDir() + "/" + fileName;
	...
	public boolean compareTextInPDFFiles(String pdfFile1, String pdfFile2) throws Exception {
		...
		try {
			...
			logError("Exception in compare text in PDF files. " + "Exception: " + e.getMessage());
			return false;
		} finally {
			pdDoc1.close();
			pdDoc2.close();
	...
	public boolean compareTextInPDFFiles(String pdfFile) throws Exception {
		...
		String baseFile = getBaseDir() + "/" + pdfFile;
		String resultFile = getBaseDir() + "/" + pdfFile;
	...
	private boolean checkIdentical(BufferedImage imgA, BufferedImage imgB) throws Exception {
		...
		// logger.logMethod(this.getClass().getName(), "checkIdentical");
	...
	private boolean checkIdentical(BufferedImage imgA, BufferedImage imgB, String imageFile, double threshold)
			throws Exception {
//		return false;    

		String currentTestID = getTestName();
		...
		if (System.getProperty("ENABLE_IMG_TOOL").equalsIgnoreCase("false"))
			return checkIdentical(imgA, imgB);

		// m_logger.logMethod(this.getClass().getName(), "checkIdentical - Image tool");

		try {
			pngDir = getResultDir() + "/" + currentTestID;
			String bigDir = getResultDir() + "/" + currentTestID + "/bigcomponents";
			String smallDir = getResultDir() + "/" + currentTestID + "/smallcomponents";
			...
			logInfo("Expected image match threshold value is: " + matchThreshold + "%");

			double percent = imgcmp.compareImages();
			logInfo("Actual image match threshold value is: " + percent + "%");

			// if (percent > 95){
			if (percent > matchThreshold) {
				return true;
			} else {
				logError("Actual image match threshold value " + percent + "%" + " which is less than expected value "
						+ matchThreshold + "%");
				return false;
			}
		} catch (Exception e) {
			// e.printStackTrace();
			logError("Exception in checkIdentical - Image tool. " + "Exception: " + e.getMessage());
			return false;
		}
	...
	/**
	 * Compare text file with static lines information, all exists in a compare to
	 * file
	 * 
	 * @param staticFilePath:    a text file contains lines of static information
	 * @param compareToFilePath: a file may contains dynamic content, that user want
	 *                           to search for whether alls lines of information in
	 *                           the static file exist in this compare file in the
	 *                           order listed in static file.
	 * @throws Exception
	 * 
	 */
	public boolean compareStaticContentInTextFiles(String compareToFilePath, String staticFilePath)
			throws Exception, IOException {
		// m_logger.logMethod(this.getClass().getName(),
		// "compareStaticContentInTextFiles", new String[]{compareToFilePath,
		// staticFilePath});
	...
	/**
	 * Copy download file to result directory.
	 *
	 * @param pdfFile PDF file name. For example, charttest.pdf
	 * 
	 */
	public void copyDownloadFileToResultDir(File downLoadFile, String changeFileNameAs) throws Exception {
		String oldFileName;
		...
		File resultFile;
		if (changeFileNameAs == null) {
			resultFile = new File(getResultDir() + "/" + oldFileName);
		} else {
			resultFile = new File(getResultDir() + "/" + changeFileNameAs);
		}

		FileUtil.copyFile(downLoadFile, resultFile);
	}

	public boolean compareTextFileWithPattern(String fileName, List<String> vSkipPatterns) throws IOException {
		String baseFilePath, resultFilePath;
		...
		return compareTextFileWithPattern(baseFilePath, resultFilePath, vSkipPatterns);
	}

	public boolean compareTextFileWithPattern(String baseFilePath, String resultFilePath, List<String> vSkipPatterns)
			throws IOException {
		BufferedReader baseBuffered = new BufferedReader(new FileReader(baseFilePath));
		BufferedReader resultBuffered = new BufferedReader(new FileReader(resultFilePath));
		String line;
		String actualLine;
		while (((line = baseBuffered.readLine()) != null) && ((actualLine = resultBuffered.readLine()) != null)) {
			if (vSkipPatterns != null)
				for (String sSkipPattern : vSkipPatterns) {
					Pattern skipPattern = Pattern.compile(sSkipPattern);
					Matcher matcher1 = skipPattern.matcher(line);
					Matcher matcher2 = skipPattern.matcher(actualLine);
					line = matcher1.replaceAll("");
					actualLine = matcher2.replaceAll("");
				}
			if (!line.equals(actualLine)) {
				logInfo("Base Line:" + line);
				logInfo("Result Line:" + actualLine);
				logError("Base Line and result line are different");
				return false;
			}
		}
		if (baseBuffered.readLine() != null) {
			logError("Base file has more lines that result file");
			return false;
		}
		if (resultBuffered.readLine() != null) {
			logError("Result file has more lines that base file");
			return false;
		}
		logInfo("Base file and result file are identical");
		return true;
	}

	/**
	 * compare two images files in base and result directory.
	 * 
	 * @param String fileName.
	 * @return true if base image is same as result image, otherwise false.
	 */
	public boolean compareTwoPNGFile(String imageFileName) throws Exception {

		String baseImage = getLogger().getBaseDir() + "/" + imageFileName;
		String resultImage = getLogger().getResultDir() + "/" + imageFileName;

		String pnGDir = getLogger().getResultDir().toString();
		...
		if (!dir2.exists())
			dir2.mkdir();
		ImageCompareFinal imgcmp = new ImageCompareFinal(baseImage, resultImage, pnGDir, false, false, true);

		// Get percent from setup.properties file
		double matchThreshold = IMG_MATCH_THRESHOLD * 100;
		logger.debug("Expected image match threshold value is: " + matchThreshold + "%");

		double percent = imgcmp.compareImages();
		logger.debug("Actual image match threshold value is: " + percent + "%");
